...
public class LBlock extends Block {
	...
	void Rotate() {
		...
		if (RotateCnt == 0) { // 'L' 에서 시작해서 반시계
			...
			tmplist[0] = new Point(list[0].x, list[0].y - 2);
			tmplist[1] = new Point(list[1].x - 1, list[1].y - 1);
			tmplist[2] = new Point(list[2].x - 2, list[2].y + 1);
			tmplist[3] = new Point(list[3].x - 1, list[3].y);
		} else if (RotateCnt == 2) {
			tmplist[0] = new Point(list[0].x, list[0].y);
			tmplist[1] = new Point(list[1].x, list[1].y);
			tmplist[2] = new Point(list[2].x + 1, list[2].y - 2);
			tmplist[3] = new Point(list[3].x + 1, list[3].y - 2);
		} else if (RotateCnt == 3) {
			tmplist[0] = new Point(list[0].x + 1, list[0].y);
			tmplist[1] = new Point(list[1].x + 2, list[1].y - 1);
