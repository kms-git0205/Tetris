...
public class JBlock extends Block {
	...
	int point = 5; // 좌측 상단 시작지점

	@Override
	boolean makeBlock() {
		super.setList(4); // 블록 개수가 4개

		// list에 L블록의 모양대로 btn설정
		list[0] = new Point(0, point);
		list[1] = new Point(1, point - 2);
	...
	void Rotate() {
		...
		// 회전 구현
		if (RotateCnt == 0) { // 'J' 에서 시작해서 반시계
			tmplist[0] = new Point(list[0].x - 1, list[0].y - 1);
			tmplist[1] = new Point(list[1].x - 2, list[1].y + 2);
			tmplist[2] = new Point(list[2].x - 1, list[2].y + 1);
			tmplist[3] = new Point(list[3].x, list[3].y);
		} else if (RotateCnt == 1) {
			tmplist[0] = new Point(list[0].x, list[0].y - 1);
			tmplist[1] = new Point(list[1].x, list[1].y - 1);
			tmplist[2] = new Point(list[2].x - 1, list[2].y);
			tmplist[3] = new Point(list[3].x - 1, list[3].y - 2);
		} else if (RotateCnt == 2) {
			tmplist[0] = new Point(list[0].x, list[0].y);
			tmplist[1] = new Point(list[1].x + 1, list[1].y - 1);
			tmplist[2] = new Point(list[2].x + 2, list[2].y - 2);
			tmplist[3] = new Point(list[3].x + 1, list[3].y + 1);
		} else if (RotateCnt == 3) {
			tmplist[0] = new Point(list[0].x + 1, list[0].y + 2);
			tmplist[1] = new Point(list[1].x + 1, list[1].y);
			tmplist[2] = new Point(list[2].x, list[2].y + 1);
			tmplist[3] = new Point(list[3].x, list[3].y + 1);
		}
